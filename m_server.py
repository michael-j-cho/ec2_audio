...
def main() -> None:
    ...
    fl.server.start_server(config={"num_rounds": int(cfg.configuartion["server_rounds"])}, strategy=strategy)
...
def get_eval_fn(model,x_train, x_test, y_train, y_test):
    ...
    def evaluate(
        ...
        global i
        i+=1
        output=str(clients_count)+','+str(epochs)+','+str(batch_size)+','+str(i)+','+str(nmi)+','+str(ari)+','+str(accuracy)+','+str(test_accuracy)+'\n'
